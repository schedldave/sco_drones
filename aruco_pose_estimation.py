    ...
    def detect(self, frame):
        ...
        if len(corners) > 0:
            for i in range(0, len(ids)):
                ...
                camera_pos = np.array(-rmat.T * np.matrix(tvec[0, 0, :]).T).flatten()
                ...
                if self.debug:
                    ...
                    str_attitude = "MARKER Attitude r=%4.1f  p=%4.1f  y=%4.1f" % (
                        math.degrees(float(marker_euler[0])),
                        math.degrees(float(marker_euler[1])),
                        math.degrees(float(marker_euler[2])),
                    )
                    cv2.putText(
                        frame,
                        str_attitude,
                    ...
                    str_attitude = "CAMERA Attitude r=%4.1f  p=%4.1f  y=%4.1f" % (
                        math.degrees(float(camera_euler[0])),
                        math.degrees(float(camera_euler[1])),
                        math.degrees(float(camera_euler[2])),
                    )
                    cv2.putText(
                        frame,
                        str_attitude,
